...
LOG_FILE="${RUN_DIR}/stage_8A_${RUN_ID}_resume_GSE8671_raw_RMA.log"
...
on_error() {
  code=$?
  {
    printf 'status\tfailed\nrun_id\t%s\nresume_step\tGSE8671_raw_RMA\nexit_code\t%s\ntimestamp\t%s\nlog\t%s\nnext_action\tCodex should inspect the GSE8671 raw-RMA resume only; do not modify data_raw\n' \
      "${RUN_ID}" "${code}" "$(date --iso-8601=seconds)" "${LOG_FILE}"
  } > "${FAILED_MARKER}"
  cp "${FAILED_MARKER}" "${STATE_FILE}"
  logger -t CRC_STAGE_8A "Stage 8A GSE8671 raw-RMA resume failed; see ${FAILED_MARKER}"
  exit "${code}"
}
trap on_error ERR
...
logger -t CRC_STAGE_8A "Stage 8A completed after single-threaded GSE8671 raw RMA; ready for Codex QC"
